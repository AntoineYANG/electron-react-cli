...
export const requireStringInput = (name, defaultValue, schema, rl) => new Promise(resolve => {
  const text = info => `${chalk`{${styles.pink} │} {${styles.cyan}.bold ${`> ${name}`}}` + (schema.required ? '' : chalk` {${styles.gray} <optional>}`) + (schema.desc ? chalk` {${styles.blue} ${`/** ${schema.desc} */`}}` : '') + chalk`{${styles.cyan} ${': '}}` + (info ? chalk`{${styles.red} ${`${info} `}}` : '') + chalk`{${styles.gray}.underline ${defaultValue.trim()}}`} `;
    ...
    rl.question(text(info), ans => {
      ...
      if (ans.length === 0) {
        if (schema.defaultValue !== undefined) {
          const t = chalk`  {${styles.blueBright} ${name}: }${schema.defaultValue}`;
...
export const requireBooleanInput = (name, defaultValue, schema, rl) => new Promise(resolve => {
  ...
  const listener = (_, key) => {
    ...
    if (/(space|up|down|left|right)/.test(key.name)) {
      ...
      const info = chalk`  {${styles.blueBright} ${`${name}: `}}{${styles.cyan} ${JSON.stringify(value)}}`;
      readline.clearLine(process.stdout, 0);
      readline.cursorTo(process.stdout, 0);
      process.stdout.moveCursor(0, -1);
...
export const requireCheckBoxInput = (name, options, schema) => new Promise(resolve => {
  const data = options.map(opt => {
    let _a; return [opt[0], opt[1], (_a = opt[2]) !== null && _a !== void 0 ? _a : opt[0]]; 
  });
  let cursor = 0;
