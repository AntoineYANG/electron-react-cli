...
const stringify = (name, tag, value = 0) => {
  ...
  const stepInfo = [ProgressTag.prepare, ProgressTag.done, ProgressTag.failed].includes(tag) ? ' ' : chalk` (${done ? chalk`{greenBright ${'\u25c8 '.repeat(done)}}` : ''}{rgb(205,185,100) \u25c8 }${waiting ? chalk`{gray ${'\u25c8 '.repeat(waiting)}}` : ''}- ${desc[tag]})`;
