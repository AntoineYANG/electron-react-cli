...
const keysNotAllowedInRoot = ['dependencies'];
/**
...
const getAllDependencies = (pkgJSON, keys) => {
  ...
  keys.forEach(key => {
    if (isRoot && keysNotAllowedInRoot.includes(key) && pkgJSON[key]) {
      logger_1.default.warn(`\`${key}\` in root \`package.json\` is found, which is not suggested. ` + 'Move them to `devDependencies` or any child package instead. ');
      return;
    }
