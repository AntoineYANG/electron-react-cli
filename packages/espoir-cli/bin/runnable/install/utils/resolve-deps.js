...
const getAvailableVersions = (name, version, lockData) => new Promise(resolve => {
  ...
  _request_1.default.npm.view(name).then(([err, data]) => {
    if (err || !data) {
      return resolve([err ?? new Error(`Failed to get data of "${name}". `), null]);
    }
    ...
    if (version === 'latest') {
      if (tagged.includes('latest')) {
        ...
        if (!which) {
          _request_1.default.npm.find(name, version).then(([err, latest]) => {
            if (err || !latest) {
    ...
    if (tagged.length) {
      Promise.all(tagged.map(td => {
        return _request_1.default.npm.find(name, td);
      })).then(list => {
        list.forEach(t => {
          if (t[1]) {
            versions.push(t[1]);
        ...
        if (versions.length === 0) {
          return resolve([new Error(`No version of "${name}" satisfies "${version}". `), null]);
        }

        return resolve([null, versions.sort((a, b) => semver.lt(a.version, b.version) ? 1 : -1)]);
      });
    } else {
      if (versions.length === 0) {
        return resolve([new Error(`No version of "${name}" satisfies "${version}". `), null]);
...
const resolveDependencies = async (dependencies, lockData, memoized = [], onProgress) => {
  ...
  const tasks = dependencies.map(async dep => {
    ...
    const target = satisfied[0]; // add it to the list
