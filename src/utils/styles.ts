...
const board = (str: string): string => {
  ...
  const formatted = lines.map((s, i) => {
    if (i === 0) {
      ...
      const head2 = (
        chalk.bgRgb(153, 204, 234).rgb(247, 237, 239)(
          '▅'.repeat(al[0]) +
          '▆'.repeat(al[1]) +
          '▇'.repeat(al[2])
        ) +
      ...
      const head2s = (
        chalk.bgRgb(153, 204, 234).rgb(250, 250, 250)(
          '▆'.repeat(al[1]) +
          '▇'.repeat(al[2])
        ) +
      ...
      return '\n' + chalk.bgRgb(0, 0, 0)(
        '  ' + chalk.rgb(153, 204, 234)(
          head + ' '.repeat((a + b) * 2 - head.length * 2) + head.split('').reverse().join('')
        ) + '  '
      ) + '\n' + (
        chalk.rgb(153, 204, 234)('██') + (
          head2 + ' '.repeat((a + b) * 2 - head.length * 2) + head2r
        ) + chalk.rgb(153, 204, 234)('██')
      ) + '\n' + (
        chalk.bgRgb(255, 245, 242).rgb(153, 204, 234)('▍ ') + (
          chalk.rgb(153, 204, 234)('██') + (
            head2s + ' '.repeat((a + b) * 2 - head.length * 2) + head2sr
          ) + chalk.rgb(153, 204, 234)('██')
        ) + chalk.rgb(255, 245, 242).bgRgb(153, 204, 234)('█▍')
      ) + '\n' + (
        chalk.bgRgb(255, 245, 242).rgb(153, 204, 234)('▍ ') + (
          chalk.bgRgb(250, 250, 250).rgb(153, 204, 234)('▍ ')
        ) + chalk.bgRgb(250, 250, 250).rgb(221, 106, 171)(
          ' '.repeat((a + b) * 2 - 4)
        ) + (
          chalk.rgb(250, 250, 250).bgRgb(153, 204, 234)('█▍')
        ) + chalk.rgb(255, 245, 242).bgRgb(153, 204, 234)('█▍')
      );
      ...
      return chalk.rgb(153, 204, 234).bgRgb(250, 250, 250)(
        before.split('').map((d, i) => i === 0 || i === 2 ? '█' : d).join('') +
        ' '.repeat(t) +
        before.slice(0, before.length - 3).split('').reverse().join('') +
        '█' + before.charAt(1) + '█'
      ) + '\n' + chalk.bgRgb(153, 204, 234).rgb(0, 0, 0)(
        tail + '▄'.repeat(t) + tail.split('').reverse().join('')
      );
    }
    return chalk.bgRgb(255, 245, 242).rgb(153, 204, 234)('▍ ') + (
      chalk.bgRgb(250, 250, 250).rgb(153, 204, 234)('▍ ')
    ) + (
      chalk.bgRgb(250, 250, 250).rgb(221, 106, 171)(fit(s, (a + b) * 2 - 4))
    ) + (
      chalk.rgb(250, 250, 250).bgRgb(153, 204, 234)('█▍')
    ) + chalk.rgb(255, 245, 242).bgRgb(153, 204, 234)('█▍');
  }).join('\n');
